...
import { NtpExpense, NtpContract } from '@/types'
...
const emptyForm = {
  ntp_contract_id: 'none', category: 'Phát sinh', description: '',
  planned_amount: '', actual_amount: '', date: new Date().toISOString().split('T')[0],
  status: 'planned' as NtpExpense['status'], note: '',
}

export default function ChiTieuNTP({ projectId, expenses, contracts, isAdmin }: Props) {
  ...
  function openEdit(e: NtpExpense) {
    setEditId(e.id)
    setForm({
      ntp_contract_id: e.ntp_contract_id || 'none', category: e.category,
      description: e.description, planned_amount: String(e.planned_amount),
      actual_amount: String(e.actual_amount), date: e.date, status: e.status, note: e.note || '',
    })
    setOpen(true)
  }
  ...
  async function handleSubmit(evt: React.FormEvent) {
    ...
    const payload = {
      ...
      ntp_contract_id: form.ntp_contract_id === 'none' ? null : form.ntp_contract_id,
              ...
              <DialogContent>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-2">
                    <Label>Hợp đồng NTP (tùy chọn)</Label>
                    <Select value={form.ntp_contract_id} onValueChange={v => setForm({ ...form, ntp_contract_id: v })}>
                      <SelectTrigger><SelectValue placeholder="-- Chọn hợp đồng --" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none">-- Không liên kết --</SelectItem>
                        {contracts.map(c => <SelectItem key={c.id} value={c.id}>{c.ntp_name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
